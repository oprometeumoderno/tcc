    ...
    \begin{abstract}
    Tarefas de aprendizado por reforço compreendem uma subárea da inteligência artificial onde um agente, inserido dentro de um ambiente, aprende
    ao interagir com ele. Decidindo as suas ações dentro deste ambiente, o agente ali inserido analisa as consequências destas
    e extrai aprendizado de sua análise, visando maximizar a recompensa, que é um número que representa a qualidade das suas ações, sendo a única forma de avaliação disponível ao agente. \henrique{aqui eu preciso dizer que a recompensa além de avaliar as ações do agente é a única forma dele saber se foi uma ação boa ou ruim, certo?}. Esta é a grande diferença entre aprendizado por reforço e outras áreas, como o aprendizado supervisionado,
    onde é apresentado, de maneira explícita e independente do agente, um retorno sobre qual deveria ser a ação correta em cada situação. No aprendizado por reforço, entretanto,
    o agente observa apenas (\henrique{reforçando o caráter não-supervisionado do aprendizado por reforço}) o efeito das suas ações, medindo a importância que suas escolhas tiveram para resolver a tarefa que lhe foi atribuída
    através do conceito de recompensa, que nada mais é do que um valor numérico que encapsula o sucesso que as consequências de suas ações tiveram para
    a resolução da tarefa. O agente, portanto, deve atualizar o seu comportamento através de um processo que envolve não só o refinamento das ações tomadas por ele
    no passado, mas também a busca por ações completamente diferente das anteriores, visando sempre a maximização da recompensa recebida. O processo
    de aprendizado por reforço, por conseguinte, pode exigir uma quantidade considerável de tempo, por ser, em essência, um processo não guiado de busca
    pela melhor solução para uma determinada tarefa dentro do espaço de soluções possíveis.
    Por isso, quando algoritmos de Aprendizado por Reforço são aplicados em tarefas onde a avaliação de uma solução possível é bastante cara (computacionalmente e em termos de tempo), tais como tarefas de robótica, é normal o uso de simuladores para, através da simulação do robô em um ambiente virtual, direcionar o processo de aprendizado antes de aplicá-lo a um robô construído no mundo real e, deste modo, acelerar 
    processo de treinamento do agente. 
    A ferramenta Gym oferece diferentes simuladores (chamados de "cenários") que tratam de problemas famosos da literatura de aprendizado por reforço, ao mesmo tempo em que disponibiliza ferramentas para submissão de soluções visando a troca de conhecimento entre pesquisadores do mundo todo. O leque de cenários oferecidos é bastante extenso; não é oferecido pelo Gym, todavia, ferramentas para a criação de cenários novos. Neste trabalho nós descrevemos o processo de criação de um \textit{framework} para criação de cenários novos e comparamos o grau de complexidade exigido por ele e por outros \textit{frameworks} disponíveis, demonstrando que não apenas os simuladores gerados automaticamente por nossa ferramenta são compatíveis com APIs padrão na área, mas também são de fácil especificação, via uma linguagem de alto-nivel, o que facilita a criação de ambientes para avaliação e comparação de diferentes algoritmos de aprendizado por reforço.
    
    \henrique{Aqui eu quero dizer que o nosso é mais simples e compatível com a API do gym. Não sei se ficou muito claro, mas eu quero que meu trabalho seja uma espécie de ponto intermediário entre o Gym (que oferece um monte de cenários pra que os caras troquem conhecimento, mas não permite criação de cenários novos e programas como o MuJoCo, por exemplo, que permitem a criação de simuladores mas não funcionam como uma ferramenta de benchmark. Obviamente meu trabalho não faz as duas coisas maravilhosamente bem, mas é uma tentativa de aproximar esses dois extremos do espectro. Essa é a principal vantagem do Barbell e e por ela que eu vou vender meu peixe. }
    \end{abstract}
