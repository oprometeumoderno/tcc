...
custo associado à adoção de um \textit{framework} para construção de simuladores
  ...
  de aprendizado por reforço através dele e de outros \textit{framworks}
...
\begin{equation}
  \pi^* = argmax_\pi\mathbb{E}[\sum_tR(S_t) | \pi]
\end{equation}

 que é a \textit{recompensa esperada total}, ou por

\begin{equation}
  \pi^* = argmax_\pi\mathbb{E}[\sum_t\gamma^tR(S_t) | \pi]
\end{equation}

que é a \textit{recompensa esperada descontada}, onde o \textit{fator de
desconto} $\gamma \in  ]0,1[$ serve para garantir a convergência do valor da
...
discutidas a seguir.
% Em
% inteligência artificial, a noção de \textit{aprendizado} trata de uma correção
% que o agente executa em si mesmo, levando em consideração seu erro e um
% coeficiente de aprendizado, que determina o impacto que o último erro terá na
% atualização do agente em um determinado momento. Há muitos algoritmos de
% aprendizado por reforço que funcionam dessa maneira, e um dos mais estudados
% deles é chamado Q-Learning.
