...
reforço são aplicáveis a uma vasta gama de problemas. De agentes que aprendem
a disputar partidas de jogos de tabuleiro a sistemas de controle de braços
robóticos que aprendem a atirar objetos, ações são tomadas em sequência em
inúmeras situações diferentes. Também foi mostrado através de vários exemplos
que simuladores são ferramentas imprescindíveis no treinamento de tais agentes,
uma vez que, apresar de possuírem suas limitações, fornecem uma poderosa
ferramenta de abstração do problema, deixando as diferentes situações pelas
quais o agente é submetido sob o controle daqueles que o treinam.


Também foi desenvolvido que ferramentas de simulação de problemas de aprendizado
por reforço podem ter diferentes graus de especificidade. Para problemas
de uma subárea específica do aprendizado de máquina, como a robótica, pode ser
mais vantajoso adotar ferramentas menos genéricas de construção de ambientes de
AR em detrimento de ferramentas de propósito mais geral, dado que
\textit{frameworks} mais específicos fornecem atalhos para a modelagem de
problemas dentro do seu contexto. Há de ser analisado, portanto, o
\textit{trade-off} entre a adoção de uma ferramenta de propósitos mais gerais,
que facilita a criação de simulações das mais variadas naturezas, mas que não
fornece atalhos que facilitem a criação de ambientações de problemas com alto
grau de especificidade, e a adoção de ferramentas mais especializadas, como o
\textit{Gazebo} para problemas de robótica ou a \textit{Unity ML-Agents Toolkit}
para problemas envolvendo jogos onde agentes competem entre si, que são bastante
específicas e provêm maneiras mais simplificadas de modelagem de ambientes
dentro do seu próprio contexto, mas que são limitadas quando o problema exige
elementos que estão fora dele.
...
aprendizado por reforço avança.
